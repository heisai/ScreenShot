...
Screen::Screen(const QString &file,QWidget *parent)
    : QWidget(parent),
      widthInfoRect(QRectF(0, 0, 0, 0)),FileName(file)
{
    ...
    QTimer Timer;
    connect(&Timer,&QTimer::timeout,[this]{qDebug()<<"11111111111111111111";});
...
{
    oncePress = false;
    int width = controlUi->width();
    int height = controlUi->height();
    controlUi->setGeometry(movePoint.x() - width, movePoint.y() + 5, width, height);
    controlUi->show();
...
void Screen::mouseMoveEvent(QMouseEvent *e)
{
    ...
    if(e->buttons() & Qt::LeftButton)   //如果左键按下
    {
        ...
        } else
        {
            ...
            if( controlUi )  //如果控制面板已经出现，则第二次以后的每一次鼠标按键都会使其暂时隐藏
                controlUi->hide();
